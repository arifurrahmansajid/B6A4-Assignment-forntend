...
export const OrderTracking = () => {
  return (
    <section id="tracking" className="relative bg-background py-24 overflow-hidden transition-colors duration-500">
      {/* Visual Accents */}
      ...
      <div className="section-container relative z-10">
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
            ...
            <div className="grid sm:grid-cols-3 gap-8 pt-4">
              {features.map((feature) => (
                <div key={feature.title} className="flex flex-col gap-4 group">
                  <div className="w-12 h-12 rounded-2xl bg-card border border-border/40 flex items-center justify-center shadow-lg transition-all group-hover:bg-primary group-hover:scale-110">
                    <feature.icon className="w-5 h-5 text-primary group-hover:text-white transition-colors" />
                  </div>
                  <div className="space-y-1">
                    <h3 className="font-black text-[9px] uppercase tracking-widest text-foreground group-hover:text-primary transition-colors">
                      {feature.title}
                    </h3>
          ...
          <div className="relative group/card">
            {/* Glow backdrop */}
            <div className="absolute -inset-10 bg-primary/10 rounded-full blur-[80px] opacity-0 group-hover/card:opacity-100 transition-opacity duration-1000" />

            <div className="relative z-10 bg-secondary/20 dark:bg-card/40 backdrop-blur-2xl rounded-[3rem] p-8 md:p-10 shadow-2xl border border-border/40 transition-colors duration-500">
              {/* Header */}
              <div className="flex items-center justify-between mb-8 pb-8 border-b border-border/40">
                <div className="space-y-1">
                  <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/60">Reference</p>
                  <p className="text-lg font-black text-foreground tracking-tight">#FH-HERO-001</p>
                </div>
                <div className="flex flex-col items-end gap-2">
                  <span className="px-4 py-2 bg-emerald-500/10 text-emerald-600 text-[10px] font-black uppercase tracking-widest rounded-xl border border-emerald-500/20 shadow-sm animate-pulse">
                    In Transit
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-0 px-2">
                {trackingSteps.map((step, index) => (
                  <div key={step.label} className="flex gap-6 md:gap-8 group/step">
                    <div className="flex flex-col items-center">
                      ...
                      {index < trackingSteps.length - 1 && (
                        <div
                          className={`w-[2px] h-10 transition-all duration-1000 delay-300 ${step.completed ? "bg-foreground" : "bg-border/40"
                            }`}
                        />
                      )}
                    </div>
                    <div className="pb-8 space-y-1">
                      <p
                        className={`text-base font-black tracking-tight transition-colors ${step.completed
                          ? "text-foreground"
              ...
              {/* Map visualizer */}
              <div
                className="relative overflow-hidden group h-28 bg-foreground rounded-[2rem] border border-white/10 flex items-center justify-center mt-4 transition-all hover:scale-[1.02] active:scale-95 cursor-pointer"
              >
                ...
                <div className="relative z-10 text-center space-y-1">
                  ...
                  <p className="text-[9px] font-black uppercase tracking-[0.2em] text-white/80">View Trajectory</p>
                </div>

                <ChevronRight className="absolute right-6 top-1/2 -translate-y-1/2 h-4 w-4 text-white/40 group-hover:translate-x-1 group-hover:text-primary transition-all" />
              </div>
            </div>

            {/* Cinematic back-shadow */}
            <div className="absolute -inset-2 -z-10 bg-primary/5 rounded-[3.5rem] blur-2xl" />
          </div>
        </div>
      </div>
    </section>
